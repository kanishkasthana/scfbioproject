//Made by Kanishk Asthana with thanks to Ms.Priyanka Dhingra for helping in I/O
...
//Updated 16 July
//Updated 18,19,20,21,22,23,26,27,30 July (A lot of changes)
...
int main(int argc,char *argv[])
{
 ...
 //First part: Calculating values of l,m and n and reference atoms
   ...
   //Part two: Finding the rotation angle between the two sets of coordinates
    ...
    checkcosphi=((xs*xsd)+(ys*ysd))/(pow((xsd*xsd)+(ysd*ysd),0.5)*pow((xs*xs)+(ys*ys),0.5));
    checksinphi=((xs*ysd)-(ys*xsd))/(pow((xsd*xsd)+(ysd*ysd),0.5)*pow((xs*xs)+(ys*ys),0.5));
